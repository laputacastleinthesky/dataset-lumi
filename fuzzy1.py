...
# Parameters
START_NUM = 2701  # Start task number
...
for i in range(TOTAL_FILES):
    ...
    # 生成txt内容
    text = f"请{robot_idx}将{quantity}个{item_idx}在{year}年{month}月{date}日{time_idx}{action_idx}{loc_idx}点位"
    texts.append(text)
...
# Generate JSON files
def generate_json(task_id, quantity, target, action_idx, item_name, task_type, year, month, date, time_idx):
    time_mapping = {
