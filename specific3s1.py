...
# Parameters
START_NUM = 7001  # Start task number
...
for i in range(TOTAL_FILES):
    ...
    # 生成txt内容（包含两个物品和两个数量）
    text = f"请将{quantity1}个{item1},{quantity2}个{item2}和{quantity3}个{item3}{action_idx}{loc_idx}点位"
    texts.append(text)
...
def generate_json(task_id, quantity1, quantity2, quantity3, item1, item2, item3, target, action_idx, task_type):
