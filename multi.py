...
# Parameters
START_NUM = 3201  # Start task number
...
for i in range(TOTAL_FILES):
    ...
    # 生成txt内容（包含两个物品和两个数量）
    text = f"{sequence_idx}{num1}订单,订单号{num2},请将{quantity1}个零件号为{num3}的{item1}和{quantity2}个零件号为{num7}的{item2}在{year}年{month}月{date}日{time_idx}由{robot_idx}从{num4}列{num5}层{num6}号货箱和{num8}列{num9}层{num10}号货箱{action_idx}{loc_idx}拣选口"
    texts.append(text)
...
def generate_json(task_id, quantity1, quantity2, item1, item2, target, action_idx, task_type, year, month, date, time_idx):
    time_mapping = {
